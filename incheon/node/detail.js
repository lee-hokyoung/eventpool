...
var $infoData = $scope.fromOntology('InfoWindows_body_contents') ? ($scope.fromOntology('InfoWindows_body_contents'))['@id'] : 'InfoWindows_body_contents';
...
for (var $attr in phone) {
	...
	switch($attr){
		case $picturesInBottomPropertyName:
			...
			'<div>';
			for($key in $value){
				$render[$attr] += '<img src="' +$value[$key]['@value']+ '" rel="noreferrer" class="phoneBot ">';
			}
			$render[$attr] += '</div>';
			$render[$attr] += '</li>';
		break;
		...
		case "http://www.w3.org/2003/01/geo/wgs84_pos#lat_long":
			...
			if(util.isArray($value)){
				$value.forEach(function(latLng){
					...
					var rndr = arr.join(",");
					var info = phone[$infoData];
					if(info && info['@value']){
    					rndr += ',\'<div id="content"><div id="siteNotice"></div>' +
    					'<h1 id="firstHeading" class="firstHeading">' + filterName(phone['@id']) + '</h1>'+
                        '<div id="bodyContent">'+
                        (info['@value'].replace("'","\\'")) +
                        '</div>'+
                        '</div>\'';
					}
					$render[$attr] += 'GoogleMapsLocations.push([' + rndr + ']);';
				});
			}else{
